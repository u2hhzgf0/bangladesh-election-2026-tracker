...
import { VoteData, NewsInsight, Candidate, ReferendumData } from './types';
import { getElectionInsights } from './services/apiService';
import { initializeSocket, onVoteUpdate, onCountdownUpdate, onInitialData } from './services/socketService';
...
const App: React.FC = () => {
  ...
  const [referendum, setReferendum] = useState<ReferendumData>({
    yes: 12540,
    no: 8430
  });
  ...
  useEffect(() => {
    ...
    const unsubscribeInitial = onInitialData((data) => {
    ...
    const unsubscribeVotes = onVoteUpdate((data) => {
      if (data.votes) setVotes(data.votes);
      if (data.referendum) setReferendum(data.referendum);
    });

    // Listen for countdown updates
    const unsubscribeCountdown = onCountdownUpdate((data) => {
      setCountdown(data);
    });
  ...
  // Fetch election insights
  useEffect(() => {
    const fetchInsights = async () => {
      const data = await getElectionInsights();
      setInsights(data);
    };
    fetchInsights();
  }, []);
  ...
  const handleManualVote = async (party: 'rice' | 'scale') => {
    // Vote is cast through the backend API
    // The socket will automatically update the UI
    try {
      const { castVote } = await import('./services/apiService');
  ...
  const handleReferendumVote = async (choice: 'yes' | 'no') => {
    // Vote is cast through the backend API
    try {
      const { castReferendumVote } = await import('./services/apiService');
      await castReferendumVote(choice);
    } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-14 sm:h-16 items-center">
            <div className="flex items-center gap-2">
              ...
              {isConnected && (
                <span className="ml-2 px-2 py-0.5 bg-green-100 text-green-700 text-[8px] font-bold rounded-full">
                  LIVE
                </span>
              )}
            </div>
